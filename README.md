...
The `sample` app is an installable APK of the same harness. Every release attaches it, and every CI build of `main` uploads it as the `sample-debug-apk` artifact.

1. **Download** the APK from the [latest release](https://github.com/susyimes/android-agent-harness/releases/latest). It is **debug-signed and sideload-only** — it is not distributed through any app store, so Android will ask you to allow installing from an unknown source. A debug build is also *debuggable*, which means anyone with adb access to your unlocked device can read the credential it stores: use a low-limit, revocable key and press **Clear credential** when you are done.
2. **Install and open** it (Android 8.0 / API 26 or newer).
3. **Chat.** Two ways:
   - *No credential at all:* just press **Send**. A deterministic offline scripted provider runs the full provider → tool → provider loop locally with zero network traffic.
   - *Your own credential:* open **Settings** in the app and paste the base URL, model, and API credential of any OpenAI-compatible chat-completions endpoint. The credential ships nowhere with the app — it is stored only in the app's private `SharedPreferences` on your device, is sent only as an `Authorization` header to the endpoint you configured, and **Clear credential** removes it.
...
- **You must enable it yourself.** The app cannot and does not switch the service on. Toggling phone mode only opens the system **Settings → Accessibility** screen, where you enable the "Agent Harness" service manually. Phone mode also requires a stored credential (the offline scripted path is chat-only).
...
`checkM0` runs the provenance/privacy audit, the JVM unit/end-to-end tests, the demo tests, and `:sample:assembleDebug`. The Android library's JVM-hosted mapper tests run with `:device-loop-android:testDebugUnitTest` (also requires the SDK).
...
- `sample`: installable Android chat app over the same core — live chat against a user-configured OpenAI-compatible endpoint (credential pasted in-app, stored only in app-private preferences), an offline scripted provider when no credential is stored, and an optional phone mode that drives this device through the accessibility service with a human approval dialog gating every high-risk action.
